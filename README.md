...
## Demo
* Create the folder `checkpoint` in the root directory, [download](https://drive.google.com/file/d/1RhP3PK2sjW0Xsh0tOGGya_wz8EcOfOtV/view?usp=sharing) our pretrained checkcpoint (53.1MB) to the folder, then run the demo:    
>```
> python demo.py
>```
>Suction area predictions should be saved in the folder `sample`, or specify the `path/to/chekcpoint` and `path/to/samples` using `-c` and `-d` args respectively.
## Training
...
>By default, RGANet read the lattest checkpoint from the folder `checkpoint`, you can also specify the checkpoint using arg `-c`:
>```
...
## Validation
>We provide both online validation and offline valiation. The online validation runs tests on all checkpoints and estimates the checkpoint that may has the best performance. Offline validation requires predictions saved to disk beforehand, i.e., run test w/ predictions written to disk before any offlline validation. Please make sure you've set desired options in `utils/configuraion.py`. Run  
>```python RGANet.py -v``` or ```python RGANet.py -test -v```  
>Refer to arg `-d` to specify the path to predictions.
## Scripts
* In addtional to the functions provided above, we also provides useful tools:

| name | illustration|
| :-----:| :----: |
| calculator.py | evaluation statisics, caculate model parameters | 
| eval_adaptor.py | splite items of validation result to separete files | 
| models.py | standalone training, validation of segmentaion models| 
